...
export function ServiceDetail({ title, tagline, description, problems, faqs }: Props) {
    ...
    <SiteLayout>
      <section className="bg-gradient-hero py-16 text-navy-foreground md:py-24">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 md:px-6 lg:grid-cols-[1.2fr_1fr]">
          <div>
            <Link to="/services" className="inline-flex items-center gap-1 text-xs font-semibold text-navy-foreground/70 hover:text-white">
              ← All Services
            </Link>
            <h1 className="mt-3 font-display text-4xl font-bold leading-tight md:text-5xl">
              {title}
            </h1>
            <p className="mt-3 text-xl font-medium text-primary-foreground/90">{tagline}</p>
            <p className="mt-5 max-w-xl text-base text-navy-foreground/80">{description}</p>
        ...
        <div className="mx-auto max-w-3xl px-4 md:px-6">
          ...
          <div className="mt-8 space-y-3">
            {faqs.map((f, i) => {
              const isOpen = open === i;
              return (
                <div key={f.q} className={`overflow-hidden rounded-2xl border bg-card shadow-soft transition ${isOpen ? "border-primary/40" : "border-border"}`}>
                  <button
